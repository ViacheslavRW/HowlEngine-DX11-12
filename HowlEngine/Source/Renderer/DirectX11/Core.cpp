     ...
     void Core::InitializeInputLayout(InputLayoutType type, ComPtr<ID3DBlob>& pBlob, ComPtr<ID3D11InputLayout>& pInputLayout, ComPtr<ID3D11Device>& pDevice)
     {
         switch (type)
         {
             case InputLayoutType::PBR:
                 {
                     ...
                     if (FAILED(coreHRes)) std::cout << "FAILED_TO_CREATE_INPUT_LAYOUT_PBR" << std::endl;
                 } break;
             case InputLayoutType::Skybox:
             {
                 ...
                 HRESULT coreHRes = pDevice->CreateInputLayout(layout, (UINT)std::size(layout), pBlob->GetBufferPointer(), pBlob->GetBufferSize(), &pInputLayout);
                 if (FAILED(coreHRes)) std::cout << "FAILED_TO_CREATE_INPUT_LAYOUT_SKYBOX" << std::endl;
             } break;
             case InputLayoutType::Box:
             {
                 D3D11_INPUT_ELEMENT_DESC layout[] =
                 {
                     { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
                 ...
                 HRESULT coreHRes = pDevice->CreateInputLayout(layout, (UINT)std::size(layout), pBlob->GetBufferPointer(), pBlob->GetBufferSize(), &pInputLayout);
                 if (FAILED(coreHRes)) std::cout << "FAILED_TO_CREATE_INPUT_LAYOUT_BOX" << std::endl;
             } break;
         }
     }
